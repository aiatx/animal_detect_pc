...
def run_mock_drone():
    ...
    while True:
        ...
        if msg.startswith("ROUTE:"):
            ...
            # --- 核心逻辑：实战沙盘预分配 ---
            valid_wps = [wp for wp in route if wp != 'A9_B1'] # 排除起飞区
            ...
            for wp in route:
                if wp == 'A9_B1':
                    print(f"  -> [状态] 正在起飞区 {wp} 爬升至安全高度...")
                    time.sleep(0.3)
                    continue

                print(f"  -> [飞行中] 全速前往目标点 {wp} ...")
