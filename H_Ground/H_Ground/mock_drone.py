...
def run_mock_drone():
    ...
    while True:
        ...
        for grid_id, tag in route_entries:
            ...
            if tag == "P" and grid_id in animal_map and grid_id not in reported_grids:
                counts = animal_map.get(grid_id, [0, 0, 0, 0, 0])
                animals_str = "".join(map(str, counts))
                if animals_str != "00000":
                    _send_udp(sock, f"REPORT:{animals_str}@{grid_id}")
                    print(f"     🎯 [识别] 上报目标: REPORT:{animals_str}@{grid_id}")
                else:
                    print("     [识别] 区域安全，未发现动物。")
